...
void ABlasterGameMode::OnMatchStateSet()
{
	...
	{
		//在服务端上遍历所有客户端的模拟角色的Controller
		//在客户端中，改变了MatchState则是使用OnRep运行
                                        ...
                                        ABlasterPlayerController* AttackerController)
{
	...
	if (AttackerPlayerState && AttackerPlayerState != VictimPlayerState && BlasterGameState)
	{
		...
		{
			ABlasterCharacter* Leader = Cast<ABlasterCharacter>(AttackerController->GetPawn());
			if (Leader)
			{
...
void ABlasterGameMode::RequestRespawn(ACharacter* ElimedCharacter, AController* ElimedController)
{
	...
	if (ElimedController)
	{
		...
		RestartPlayerAtPlayerStart(ElimedController, Actors[Selection]);
	}
}
...
void ABlasterGameMode::PlayerLeftGame(ABlasterPlayerState* PlayerLeaving)
{
	...
	ABlasterCharacter* CharacterLeaving = PlayerLeaving->GetPawn<ABlasterCharacter>();
	if (CharacterLeaving)
	{
